...
foreach ($envFiles as $envFile) {
    if (file_exists(dirname(__DIR__) . '/' . $envFile)) {
        $dotenv = Dotenv\Dotenv::createMutable(dirname(__DIR__), $envFile);
        $dotenv->load();
        $envLoaded = true;
    }
}
...
if (!$envLoaded) {
    throw new LogicException('Failed to load environment configuration ".env" or ".env.local". Try copying the ".env.sample" file.');
}
